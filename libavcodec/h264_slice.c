...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    if (h->ps.pps->cabac)
    {
        ...
        #if gly_display
            int temp_qp = sl->qscale;
            if (temp_qp < 30) temp_qp = 30;
            if (temp_qp > 50) temp_qp = 50;
            avctx->all_measure.encode_quality = 5 - 4 * ( temp_qp - 30 ) /(float) (50-30);
        ...
        for (;;)
        {
            ...
            if (sl->mb_x + sl->mb_y * h->mb_width >= sl->next_slice_idx)
            {
                av_log(h->avctx, AV_LOG_ERROR, "Slice overlaps with next at %d\n",
                       sl->next_slice_idx);
                #if gly_display
                avctx->all_measure.corect_quality =  5 * sl->mb_y  / (float) h->mb_width ;
                #endif
