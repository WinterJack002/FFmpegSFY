                                ...
                                int mb_x, int mb_y)
{
    ...
    { // FIXME merge with the stuff in fill_caches?
        ...
        mba_xy = mb_xy - 1;
        if ((mb_y & 1) && h->slice_table[mba_xy] == sl->slice_num && MB_FIELD(sl) == !!IS_INTERLACED(h->cur_pic.mb_type[mba_xy]))
            mba_xy += h->mb_stride;
        if (MB_FIELD(sl))
        {
            mbb_xy = mb_xy - h->mb_stride;
            if (!(mb_y & 1) && h->slice_table[mbb_xy] == sl->slice_num && IS_INTERLACED(h->cur_pic.mb_type[mbb_xy]))
                mbb_xy -= h->mb_stride;
        }
        else
...
#define CC &sl->cabac
    ...
    significant_coeff_ctx_base = sl->cabac_state + significant_coeff_flag_offset[MB_FIELD(sl)][cat];
    last_coeff_ctx_base = sl->cabac_state + last_coeff_flag_offset[MB_FIELD(sl)][cat];
    abs_level_m1_ctx_base = sl->cabac_state + coeff_abs_level_m1_offset[cat];
