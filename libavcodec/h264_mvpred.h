...
#define SET_DIAG_MV(MV_OP, REF_OP, XY, Y4)                                        \
    ...
    return h->cur_pic_ptr->ref_index[list][4 * xy + 1 + (y4 & ~1)] REF_OP;
        ...
        if (topright_ref == PART_NOT_AVAILABLE && i >= scan8[0] + 8 && (i & 7) == 4 && sl->ref_cache[list][scan8[0] - 1] != PART_NOT_AVAILABLE)
        {
...
static av_always_inline void pred_pskip_motion(const H264Context *const h,
                                               H264SliceContext *sl)
{
    DECLARE_ALIGNED(4, static const int16_t, zeromv)
    [2] = {0};
    DECLARE_ALIGNED(4, int16_t, mvbuf)
    [3][2];
...
static void fill_decode_neighbors(const H264Context *h, H264SliceContext *sl, int mb_type)
{
    ...
    sl->topleft_type = h->cur_pic.mb_type[topleft_xy]; // 得到邻居宏块的 mb_type
    ...
    if (FMO)
    {                                                    // 灵活宏块排序
        if (h->slice_table[topleft_xy] != sl->slice_num) // 如果 topleft_xy 不属于当前 slice，则 topleft_type = 0（无效）
            sl->topleft_type = 0;
...
static void fill_decode_caches(const H264Context *h, H264SliceContext *sl, int mb_type)
{
    ...
    if (!IS_SKIP(mb_type))
    {
        if (IS_INTRA(mb_type))
        {
            int type_mask = h->ps.pps->constrained_intra_pred ? IS_INTRA(-1) : -1; // samples_available 是一个 16-bit 掩码，表示邻近像素是否可用（用于 Intra 预测）
            sl->topleft_samples_available =
                sl->top_samples_available =
                    sl->left_samples_available = 0xFFFF; // 0xFFFF：默认所有像素可用
            ...
            if (IS_INTERLACED(mb_type) != IS_INTERLACED(left_type[LTOP])) // 当前和左宏块 场/帧编码模式是否一致
            {
        ...
        nnz_cache = sl->non_zero_count_cache; // 存储邻近宏块的 非零 DCT 系数数量
        if (top_type)
        {
    ...
    {
        int list; // 前向参考帧和后向参考帧列表
        int b_stride = h->b_stride;
        for (list = 0; list < sl->list_count; list++)
        {
            int8_t *ref_cache = &sl->ref_cache[list][scan8[0]];
            int8_t *ref = h->cur_pic.ref_index[list];
            int16_t(*mv_cache)[2] = &sl->mv_cache[list][scan8[0]]; // 从 mv 中提取邻近宏块的运动矢量，并填充到 mv_cache
            int16_t(*mv)[2] = h->cur_pic.motion_val[list];
            if (!USES_LIST(mb_type, list)) // 判断当前宏块是否使用当前的参考列表
                continue;
            av_assert2(!(IS_DIRECT(mb_type) && !sl->direct_spatial_mv_pred));

            if (USES_LIST(top_type, list))
            { // 当前参考列表是否使用上方（Top）相邻的宏块
                const int b_xy = h->mb2b_xy[top_xy] + 3 * b_stride;
                AV_COPY128(mv_cache[0 - 1 * 8], mv[b_xy + 0]); // 将运动向量从参考帧的相应位置复制到 mv_cache 中
                ref_cache[0 - 1 * 8] =
            ...
            { // 如果当前宏块不使用顶部参考列表，清零 mv_cache 并设置 ref_cache 为 LIST_NOT_USED 或 PART_NOT_AVAILABLE
