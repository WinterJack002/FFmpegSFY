...
{
    MPEGTS_PES,     // 实际音视频数据
    MPEGTS_SECTION, // PSI 数据：PAT、PMT、SDT 等
    MPEGTS_PCR,     // 时间同步基准参照
};
...
static const AVOption options[] = {
    MPEGTS_OPTIONS,
    {"fix_teletext_pts", "try to fix pts values of dvb teletext streams", offsetof(MpegTSContext, fix_teletext_pts), AV_OPT_TYPE_BOOL, {.i64 = 1}, 0, 1, AV_OPT_FLAG_DECODING_PARAM},
    {"ts_packetsize", "output option carrying the raw packet size", offsetof(MpegTSContext, raw_packet_size), AV_OPT_TYPE_INT, {.i64 = 0}, 0, 0, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY},
    {"scan_all_pmts", "scan and combine all PMTs", offsetof(MpegTSContext, scan_all_pmts), AV_OPT_TYPE_BOOL, {.i64 = -1}, -1, 1, AV_OPT_FLAG_DECODING_PARAM},
    {"skip_unknown_pmt", "skip PMTs for programs not advertised in the PAT", offsetof(MpegTSContext, skip_unknown_pmt), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM},
    {"merge_pmt_versions", "re-use streams when PMT's version/pids change", offsetof(MpegTSContext, merge_pmt_versions), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM},
    {"skip_changes", "skip changing / adding streams / programs", offsetof(MpegTSContext, skip_changes), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, 0},
    {"skip_clear", "skip clearing programs", offsetof(MpegTSContext, skip_clear), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, 0},
    {"max_packet_size", "maximum size of emitted packet", offsetof(MpegTSContext, max_packet_size), AV_OPT_TYPE_INT, {.i64 = 204800}, 1, INT_MAX / 2, AV_OPT_FLAG_DECODING_PARAM},
    {NULL},
};
...
static const AVOption raw_options[] = {
    MPEGTS_OPTIONS,
    {"compute_pcr", "compute exact PCR for each transport stream packet", offsetof(MpegTSContext, mpeg2ts_compute_pcr), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, AV_OPT_FLAG_DECODING_PARAM},
    {"ts_packetsize", "output option carrying the raw packet size", offsetof(MpegTSContext, raw_packet_size), AV_OPT_TYPE_INT, {.i64 = 0}, 0, 0, AV_OPT_FLAG_DECODING_PARAM | AV_OPT_FLAG_EXPORT | AV_OPT_FLAG_READONLY},
    {NULL},
};
...
#if WINTER_TS_ERROR
    pkt->pkt_error_flag = pes->pes_error_flag;
    pkt->pkt_error_pos = pes->error_pos;
    pkt->error_sum_per_pkt = pes->error_sum_per_pes;
#endif
                            ...
                            int64_t pos)
{
    ...
    int ret, len;
#if WINTER_TS_ERROR
    if (pes->ts_error_flag)
    {
        pes->error_sum_per_pes += buf_size;
    }

    if (pes->ts_error_flag && pes->is_first_error)
    {
        pes->error_pos = pes->data_index; // 有错 pkt 标记位置
        pes->is_first_error = 0;          // 标记已经处理过
    }
#endif
    if (!ts->pkt)
        return 0;
    ...
    while (buf_size > 0)
    {
        ...
        case MPEGTS_PESHEADER_FILL:
            ...
            if (pes->data_index == pes->pes_header_size)
            {
                ...
                if (pes->ts->fix_teletext_pts && (pes->st->codecpar->codec_id == AV_CODEC_ID_DVB_TELETEXT || pes->st->codecpar->codec_id == AV_CODEC_ID_DVB_SUBTITLE))
                {
                    AVProgram *p = NULL;
                    int pcr_found = 0;
        ...
        case MPEGTS_PAYLOAD:
            do
            {
                ...
                if (pes->PES_packet_length && pes->PES_packet_length + PES_START_SIZE > pes->pes_header_size) // pes 剪掉 pes 头剩余的音视频数据最大值
                    max_packet_size = pes->PES_packet_length + PES_START_SIZE - pes->pes_header_size;

                if (pes->data_index > 0 && // pes 拼帧内部结束记录
                    pes->data_index + buf_size > max_packet_size)
                {

                    ret = new_pes_packet(pes, ts->pkt); // 将当前 PES 数据包封装成一个 AVPacket，并重置 PES 上下文的状态，以便处理下一个 PES 数据包。
                    if (ret < 0)
                        return ret;
                ...
                else if (pes->data_index == 0 && // pes 拼帧开始，
                ...
                memcpy(pes->buffer->data + pes->data_index, p, buf_size); // 拼帧函数
                pes->data_index += buf_size;                              // 标记位置偏移
                ...
                {
                    ts->stop_parse = 1;
                    // if (pes->av_pkt_error_flag)
                    // {
                    //     pes->error_pos = pes->ts_packet_pos; // 有错 pkt 标记位置
                    // }
                    // ts->pkt->av_pkt_error_flag = pes->av_pkt_error_flag;
                    // ts->pkt->error_pos = pes->error_pos;
                    ret = new_pes_packet(pes, ts->pkt);
                    pes->state = MPEGTS_SKIP;
                    if (ret < 0)
                        return ret;
                                      ...
                                      int stream_identifier, int pmt_stream_idx, struct Program *p)
{
    ...
    else if (pmt_stream_idx < p->nb_streams)
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet, int64_t pos)
{
    ...
    if (is_start)
    {
        ...
        {
            pes_frame_count++;
            printf("[mpegts.c/handle_packet]:这是 音/视频/字幕/ 第 %d 帧\n", pes_frame_count);
            PESContext *pcc = tss->u.pes_filter.opaque;
            pcc->pes_error_flag = 0;
            pcc->error_pos = 0;
            pcc->is_first_error = 1;
            // if (pcc->state == MPEGTS_PAYLOAD)
            pcc->error_sum_per_pes = 0;
            // printf("当前的 is_error_flag 是： %d \n", pcc->is_first_error);
        }
    }
#endif
    tss = ts->pids[pid];
    ...
    if (!cc_ok)
    {
        // printf("[mpegts.c/handle_packet]:cc_ok 不正确！！！\n");
...
#endif
            // printf("[mpegts.c/handle_packet]:pc->pes_error_flag 是：%d\n\n", pc->pes_error_flag);
        }
    }
#if WINTER_TS_ERROR
    else
    ...
    else
    {
        // if (is_start)
        // {
        //     int offset_from_packet = p - packet;
        //     PESContext *pc = tss->u.pes_filter.opaque;
        //     pc->cur_pes_start_pos = pos + offset_from_packet;
        // }
        int ret;
        // Note: The position here points actually behind the current packet.
        if (tss->type == MPEGTS_PES)
        {
            // int temp = (int)(p_end - p);
...
static int mpegts_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    // printf("函数指针调用 mpegts_read_packet 从 formatcontext 读取包！\n");

    pkt->size = -1;
    ts->pkt = pkt;
    ret = handle_packets(ts, 0); // 组帧 pes 包的错误情况？
    if (ret < 0)
    {
