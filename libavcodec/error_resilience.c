...
void ff_er_frame_end(ERContext *s, int *decode_error_flags)
{
    ...
    if (s->avctx->codec_id == AV_CODEC_ID_MPEG2VIDEO && (FFALIGN(s->avctx->height, 16) & 16) && atomic_load(&s->error_count) == 3 * s->mb_width * (s->avctx->skip_top + s->avctx->skip_bottom + 1))
    {
        for (mb_x = 0; mb_x < s->mb_width; mb_x++)
        {
...
#if WINTER_FORCE_INTER
    if (s->avctx->force_inter)
        is_intra_likely = 0;
#endif
