...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size)
{
    ...
    if ((ret < 0 || h->er.error_occurred) && h->cur_pic_ptr) // 表示解码该 slice 发生错误，标记该 slice 是错误的
    {
    ...
    if (!FIELD_PICTURE(h) && h->current_slice && h->enable_er)
    {
        ...
        if (use_last_pic) // 没有参考帧，使用上一帧保存的数据
        {
...
#endif
        if (decode_error_flags) // 标记该帧经过错误修复后的
        {
            if (h->cur_pic_ptr->decode_error_flags)
            {
                atomic_int *decode_error = (atomic_int *)h->cur_pic_ptr->decode_error_flags->data;
...
double calc_laplacian_variance(AVFrame *frame) {
    ...
    int start_row = height - n - 1;
    int end_row = height - 2;
    ...
    for (int i = start_row; i <= end_row; ++i) {
        for (int j = 1; j < width - 1; ++j) {
            int a = y[(i-1) * stride + j];
            int b = y[i * stride + (j-1)];
            int c = y[i * stride + j];
            int d = y[i * stride + (j+1)];
            int e = y[(i+1) * stride + j];
            int lap = a + b + d + e - 4 * c;
            double f = (double)lap;
            sum += f;
            sum2 += f * f;
    ...
    result = sum2/ 600.0 / count - mean * mean;
                             ...
                             int *got_frame, AVPacket *avpkt)
{
    ...
    int ret;
    #if gly_display
    avctx->all_measure.ts_correct = avctx->internal->in_pkt->ts_correct;
    #endif
    ...
    #if gly_display
    avctx->all_measure.clarity = calc_laplacian_variance(pict);
