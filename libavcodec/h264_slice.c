...
static int h264_init_ps(H264Context *h, const H264SliceContext *sl, int first_slice)
{
    ...
    must_reinit = (h->context_initialized &&
                   (16 * sps->mb_width != h->avctx->coded_width || 16 * sps->mb_height != h->avctx->coded_height || h->cur_bit_depth_luma != sps->bit_depth_luma || h->cur_chroma_format_idc != sps->chroma_format_idc || h->mb_width != sps->mb_width || h->mb_height != sps->mb_height));
    if (h->avctx->pix_fmt == AV_PIX_FMT_NONE || (non_j_pixfmt(h->avctx->pix_fmt) != non_j_pixfmt(get_pixel_format(h, 0))))
        must_reinit = 1;
...
static int h264_select_output_frame(H264Context *h)
{
    ...
    if (cur->f->pict_type == AV_PICTURE_TYPE_B || (h->last_pocs[H264_MAX_DPB_FRAMES - 2] > INT_MIN && h->last_pocs[H264_MAX_DPB_FRAMES - 1] - (int64_t)h->last_pocs[H264_MAX_DPB_FRAMES - 2] > 2))
                            ...
                            const H2645NAL *nal, int first_slice)
{
    ...
    if (h->sei.recovery_point.recovery_frame_cnt >= 0)
    {
        ...
        if (h->recovery_frame < 0 || av_mod_uintp2(h->recovery_frame - h->poc.frame_num, h->ps.sps->log2_max_frame_num) > sei_recovery_frame_cnt)
        {
                           ...
                           const H2645NAL *nal)
{
    ...
    sl->resync_mb_y = sl->mb_y = (sl->first_mb_addr / h->mb_width) << FIELD_OR_MBAFF_PICTURE(h);
    ...
    if (h->slice_row[sl->slice_num & (MAX_SLICES - 1)] + 3 >= sl->resync_mb_y && h->slice_row[sl->slice_num & (MAX_SLICES - 1)] <= sl->resync_mb_y && sl->slice_num >= MAX_SLICES)
    {
    ...
    if (h->avctx->debug & FF_DEBUG_PICT_INFO)
    {
        av_log(h->avctx, AV_LOG_DEBUG,
               ...
               sl->slice_num,
               (h->picture_structure == PICT_FRAME ? 'F' : h->picture_structure == PICT_TOP_FIELD ? 'T'
                                                                                                  : 'B'),
...
static void predict_field_decoding_flag(const H264Context *h, H264SliceContext *sl)
{
    ...
    int mb_type = (h->slice_table[mb_xy - 1] == sl->slice_num) ? h->cur_pic.mb_type[mb_xy - 1] : (h->slice_table[mb_xy - h->mb_stride] == sl->slice_num) ? h->cur_pic.mb_type[mb_xy - h->mb_stride]
                                                                                                                                                         : 0;
...
static int filter_test(const H264Context *h, H264SliceContext *sl)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        sum_currentx += *(dest_y + i);
        sum_currenty += *(dest_y + i * linesize[0]);
        sum_currentx2 += (*(dest_y + i)) ^ 2;
        sum_currenty2 += (*(dest_y + i * linesize[0])) ^ 2;

        if (mb_x != 0)
        { // 如果不是第一列
            if (*(dest_y + i * linesize[0]) == *(dest_y + i * linesize[0] - 1))
            {
                y_diff_left = 0;
            }
            else
            {
                y_diff_left += abs((*(dest_y + i * linesize[0]) - *(dest_y + i * linesize[0] - 1)) * 100 / (*(dest_y + i * linesize[0] - 1) == 0 ? *(dest_y + i * linesize[0]) : *(dest_y + i * linesize[0] - 1)));
            }
            sum_lefty += *(dest_y + i * linesize[0] - 1);
            sum_lefty2 += (*(dest_y + i * linesize[0] - 1)) ^ 2;
        }

        if (mb_y != 0)
        { // 不是第一行
            if (*(dest_y + i) == *(dest_y - linesize[0] + i))
            {
                y_diff_up = 0;
            }
            else
            {
                y_diff_up += abs((*(dest_y + i) - *(dest_y - linesize[0] + i)) * 100 / (*(dest_y - linesize[0] + i) == 0 ? *(dest_y + i) : *(dest_y - linesize[0] + i)));
            }
            sum_upx += *(dest_y - linesize[0] + i);
            sum_upx2 += (*(dest_y - linesize[0] + i)) ^ 2;
        }
    }
    int current_dx = (sum_currentx2 - sum_currentx ^ 2 / 16) / 16;
    int up_dx = (sum_upx2 - sum_upx ^ 2 / 16) / 16;
    int a = abs((current_dx * 100 - up_dx * 100) / (up_dx != 0 ? up_dx : 1));

    int current_dy = (sum_currenty2 - sum_currenty ^ 2 / 16) / 16;
    int left_dy = (sum_lefty2 - sum_lefty ^ 2 / 16) / 16;
    int b = abs((current_dy * 100 - left_dy * 100) / (left_dy != 0 ? left_dy : 1));

    y_diff = (y_diff_left + y_diff_up) / 32;

    if (y_diff > 2 && (a + b) / 2 > 100)
    { // y_diff亮度，
        flag = -1;
    }
...
static int decode_slice(struct AVCodecContext *avctx, void *arg)
{
    ...
    ret = alloc_scratch_buffers(sl, sl->linesize); // 为每个 Slice 分配临时缓冲区，临时存储解码时产生的图像块、运动矢量、残差等
    if (ret < 0)
        return ret;
    ...
    sl->is_complex = FRAME_MBAFF(h) || h->picture_structure != PICT_FRAME || // 判断当前的 Slice 是否复杂（同时使用帧模式和场模式）
    ...
    if (h->ps.pps->cabac) // 从 PPS 判断是否使用 CABAC
    {
        ...
        ff_h264_init_cabac_states(h, sl); // 初始化 CABAC 的上下文状态表
        ...
        for (;;)
        {
            ...
            if (++sl->mb_x >= h->mb_width)
            {
                ...
                if (sl->mb_y >= h->mb_height)
                {
                    ...
                    if (get_bits_left(&sl->gb) == 0 || get_bits_left(&sl->gb) > 0 && !(h->avctx->err_recognition & AV_EF_AGGRESSIVE))
                    {
...
finish:
    ...
    if (er_flag == 1 || er_flag == 2)
    {
        ...
        if (h->custom_err_file && h->ffmpeg_err_file)
        {
            ...
            fprintf(h->ffmpeg_err_file,
                    "%s{\"frame\":%d,\"x\":%d,\"y\":%d}",
                    prefix, sl->error_log_frame_count, er_x, er_y);
    ...
    if (er_flag == 1)
    { // 有错
        er_add_slice(sl, resync_mb_x, resync_mb_y, last_err_x, last_err_y, ER_MB_ERROR);
#if gly_new_residual
...
int ff_h264_execute_decode_slices(H264Context *h)
{
    ...
    {
        av_assert0(context_count > 0); // checked above, exit if context_count < 0
