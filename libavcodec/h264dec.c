...
static int decode_nal_units(H264Context *h, const uint8_t *buf, int buf_size) // buf一般是AVPacket->data; //buf_size一般是AVPacket->size

{
    ...
    ret = ff_h2645_packet_split(&h->pkt, buf, buf_size, avctx, h->is_avc, h->nal_length_size,
                                avctx->codec_id, 0, 0); // h->is_avc = 0:带起始码, Annex-B 格式； h->is_avc = 1:不带起始码，AVC 格式
    if (ret < 0)
    {
    ...
    for (i = 0; i < h->pkt.nb_nals; i++) // pkt.nb_nals 代表一个 AVPacket 中的 NAL 个数
    {
        H2645NAL *nal = &h->pkt.nals[i];
        ...
        {
        case H264_NAL_IDR_SLICE:
            if ((nal->data[1] & 0xFC) == 0x98) // 检查 IDR 帧是否包含非法的 inter 预测模式
            {
            ...
            if (!idr_cleared) // 清空参考帧列表，确保 IDR 帧之后的帧不会引用过去的 P/B 帧
            {
        ...
        case H264_NAL_SLICE:
            ...
            if ((err = ff_h264_queue_decode_slice(h, nal))) // 初始化 error_status_table (错误掩盖函数)
            {
            ...
            if (h->current_slice == 1) // 只有在第一片 Slice 解码时才进行多线程解码的设置
            {
                if (avctx->active_thread_type & FF_THREAD_FRAME &&
                    i >= nals_needed && !h->setup_finished && h->cur_pic_ptr) // 帧级多线程初始化(SFY 的 TS 视频未开启，因为就一个 slice)
                {
                    ff_thread_finish_setup(avctx); // 完成多线程解码的初始化
                    h->setup_finished = 1;
                }

                if (h->avctx->hwaccel && // 启用硬件加速
                    (ret = FF_HW_CALL(h->avctx, start_frame, buf, buf_size)) < 0)
                    goto end;
            }

            max_slice_ctx = avctx->hwaccel ? 1 : h->nb_slice_ctx; // 当启用硬件加速解码时，硬件通常只能一次处理整个帧，而不能像软件解码一样分割成多个 Slice 进行并行处理
            if (h->nb_slice_ctx_queued == max_slice_ctx)          // 如果已经达到最大 slice 处理上下文数（nb_slice_ctx_queued）
            {
                if (h->avctx->hwaccel) // 如果使用硬件加速，，则：调用 FF_HW_CALL 使用硬件加速
                {
...
#endif
        NULL},
    .caps_internal = FF_CODEC_CAP_EXPORTS_CROPPING | FF_CODEC_CAP_ALLOCATE_PROGRESS | FF_CODEC_CAP_INIT_CLEANUP,
